...
/**
This plugin adds JAXB Annotations to all MyBatis Generator generated model
classes specifying XmlAccessType.FIELD as the sereialization strategy.
...
<ul>
<li>All BaseRecord classes (and PrimaryKey classes, if generated), are annotated
with the <code>@XmlAccessorType(XmlAccessType.FIELD)</code> JAXB annotation.</li>
<li>Class fields corresponding to LOB columns in the BaseRecord classes and
...
<ul>
<li>Marshal class fields corresponding to LOB columns.</li>
<li>Add any number of JAXB annotations to the Java classes or Java class
fields individually.</li>
</ul>

<p />Note that the explicitly added JAXB annotations will only be applied to
those java classes and java class fields that are explicitly specified by the
user, using the nested &lt;property&gt; elements. All other classes and java
class fields will continue to follow the default rules mentioned above.<br/>
Furthermore, the <code>@XmlAccessorType(XmlAccessType.FIELD)</code> will always
be applied to all generated model classes, whether or not explicitly specified.

<p />&nbsp;<p /><b>To Marshal class fields corresponding to LOB columns</b>
<br />To marshal class fields corresponding to LOB columns, specify a nested
...
marshalling of all LOB column java fields, in all model classes (BaseRecord,
PrimaryKey, and RecordWithBlob).</b>

<p />&nbsp;<p /><b>To Add any number of JAXB annotations to the Java classes
or Java class fields individually</b>
<br/>
Any number of JAXB annotations too can be added to individual java class fields
and java classes using nested &lt;property&gt; elements. 
<br/> To specify the JAXB annotations that apply to a particular java class
field, specify the fully qualified (see below) table column name from which this
java class field is derived in the "name" attribute of the nested
&lt;property&gt; element. Specify the JAXB annotations applicable to this
java class field, in the corresponding "value" attribute of the same nested
&lt;property&gt; element.
...
annotations applicable to a single Java class or a single Java class field.</li>
...
<p />Finally, note that the MyBatis Generator could generate between 1
to 3 classes corresponding to a single database table. (The number of
classes generated for a single database table is determined by the
<b>defaultModelType</b> attribute specified in the
<b>&lt;context ...&gt;</b> element of the MyBatis Generator XML
configuration file).
<br />All explicitly specified annotations for a given
table will be applied to all generated class corresponding to that
table. Of course, the *WithBLOBs tables would not be annotated unless
the <b>marshalBlobColumns</b> property is set to <b>true</b>.
<br />When no explicit annotations are specified for a table, the default
rules mentioned above will be applicable.<br />

<p />&nbsp;<p /><b>Examples</b>
<br />
For example, to specify that the "userId" java class field in the "Users"
class, generated from the "user_id" column in the "users" table in the
...
above-mentioned "Users" table should be annotated as shown below:<br/> <code><pre>
...
<p/>Note that this plugin class adds the @XmlAccessorType(XmlAccessType.FIELD)
...
&lt;context ...."&gt;
	...
	&lt;plugin type="org.mybatis.generator.plugins.annotations.FieldTypeJaxbAnnotations"&gt;
		...
		&lt;property name='public.UsErs' value='@XmlRootElement(name="usERs")~@XmlAccessorType(XmlAccessType.FIELD)~@XmlAccessorOrder(XmlAccessOrder.ALPHABETICAL)' /&gt;
...
<p/>This class does not support the marshalling and unmarshalling of the auto
generated Example Classes.

@author Mahiar Mody
@version 1.0.0
@see org.mybatis.generator.plugins.jaxbAnnotations.AbstractJaxbAnnotations  AbstractJaxbAnnotations
@see org.mybatis.generator.plugins.jaxbAnnotations.PropertyTypeJaxbAnnotations  PropertyTypeJaxbAnnotations
*/
public class FieldTypeJaxbAnnotations extends AbstractJaxbAnnotations
{
	public static final String XML_ACCESS_TYPE_FIELD = "@XmlAccessorType(XmlAccessType.FIELD)";

	public FieldTypeJaxbAnnotations()
	{
		super();
	}
	...
	/**
	Returns the <code>static final String XML_ACCESS_TYPE_FIELD</code>
	indicating that the XmlAccessType.FIELD annotation will be used to
	decorate all auto generated model classes. 

	@return  the <code>String</code> constant <code>XML_ACCESS_TYPE_FIELD</code>
			 of this class 
	*/
	@Override
	public String getXmlAccessTypeConstant()
	{
		return XML_ACCESS_TYPE_FIELD;
	}


	@Override
	public boolean modelFieldGenerated(Field javaField, TopLevelClass topLevelClass, IntrospectedColumn introspectedColumn, IntrospectedTable introspectedTable, Plugin.ModelClassType modelClassType)
	{
		...
		if(!marshalBlobColumns && introspectedColumn.isBLOBColumn())
		{
			...
			adding the "@XmlTransient" attribute to their BLOB columns becomes a must.
			That's what the "if" condition below is checking.
			*/
			if(modelClassType != Plugin.ModelClassType.RECORD_WITH_BLOBS)
				annotateJavaElement(topLevelClass, javaField, "@XmlTransient");
		...
		{
			annotateJavaElement(topLevelClass, javaField, properties.getProperty(mapIgnoreCasePropLookup.get(fullyQualifiedTableFieldName)));
