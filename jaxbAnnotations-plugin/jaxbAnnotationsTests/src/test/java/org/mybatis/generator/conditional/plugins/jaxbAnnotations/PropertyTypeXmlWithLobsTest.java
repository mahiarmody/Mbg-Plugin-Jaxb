...
import conditional.propertyType.withLobs.modelDto.*; //these are our mbg generated compiled model classes that we want to test.
import conditional.propertyType.withLobs.clientDao.*;
...
public class PropertyTypeXmlWithLobsTest
{
	...
	public void testInheritedJaxbComponents() throws XPathExpressionException, JAXBException
	{
		...
		{
			UserTutorialMapper mapper = sqlSession.getMapper(UserTutorialMapper.class);
			userTutorialBlobs = mapper.selectByPrimaryKey(Integer.valueOf(1));
		}
		finally
		{
			sqlSession.close();
