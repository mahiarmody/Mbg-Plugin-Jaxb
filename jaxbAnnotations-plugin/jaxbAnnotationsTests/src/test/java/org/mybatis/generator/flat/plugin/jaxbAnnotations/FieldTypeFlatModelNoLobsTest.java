...
public class FieldTypeFlatModelNoLobsTest
{
	@Test
	...
	public void testNarrativeFieldNotAnnotatedInBlobsClass()
	{
		...
		Assert.assertNull("UserTutorial.narrative java field should NOT be annotated with @XmlElement, but it is.", annt);

		annt = fldNarrative.getAnnotation(XmlTransient.class);
		Assert.assertNotNull("UserTutorial.narrative java field should be annotated with @XmlTransient, but it isn't.", annt);

		annt = fldSummary.getAnnotation(XmlTransient.class);
		Assert.assertNotNull("UserTutorial.summary java field should be annotated with @XmlTransient, but it isn't.", annt);

		annt = fldVideo.getAnnotation(XmlTransient.class);
		Assert.assertNotNull("UserTutorial.video java field should be annotated with @XmlTransient, but it isn't.", annt);
	}
}
