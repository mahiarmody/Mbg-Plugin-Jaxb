...
public class FieldTypePresenceNoLobsTest
{
	...
	public void testXmlRootElementPresence()
	{
		...
		JaxbAnnotationsTestUtils.checkAccessTypeFieldClassAnnotation(UsersToSkills.class);
	...
	@Test
	public void testFieldLevelImplicitXmlTransientAnnotations()
	{
		...
		Assert.assertNotNull("@XmlTransient annotation missing from UserPhotos.photo java class field", annt);


		annt = fldBlogTxt.getAnnotation(XmlTransient.class);
		Assert.assertNotNull("@XmlTransient annotation missing from UserBlog.blogText java class field", annt);
	}
}
