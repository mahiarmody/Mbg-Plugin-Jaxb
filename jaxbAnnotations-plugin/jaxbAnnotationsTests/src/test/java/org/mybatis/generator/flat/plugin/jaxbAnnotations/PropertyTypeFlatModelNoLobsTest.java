...
public class PropertyTypeFlatModelNoLobsTest
{
	@Test
	...
	@Test
	public void testGetNarrativeSummaryVideoMethodsAnnotatedWithXmlTransient()
	{
		...
		Assert.assertNull("UserTutorial.getNarrative() method should NOT be annotated with @XmlElement, but it is.", annt);

		annt = getNarrative.getAnnotation(XmlTransient.class);
		Assert.assertNotNull("UserTutorial.getNarrative() method should be annotated with @XmlTransient, but it isn't.", annt);

		annt = getSummary.getAnnotation(XmlTransient.class);
		Assert.assertNotNull("UserTutorial.getSummary() method should be annotated with @XmlTransient, but it isn't.", annt);

		annt = getVideo.getAnnotation(XmlTransient.class);
		Assert.assertNotNull("UserTutorial.getVideo() method should be annotated with @XmlTransient, but it isn't.", annt);
	}
}
