...
package org.mybatis.generator.flat.plugins.jaxbAnnotations;
...
import flat.propertyType.noLobs.modelDto.*; //these are our mbg generated compiled model classes that we want to test.
import flat.propertyType.noLobs.clientDao.*;
...
public class PropertyTypeXmlNoLobsTest
{
	...
	@Test
	public void testLobsNotMarshalled() throws XPathExpressionException, JAXBException
	{
		String clobStrNarrative = null, clobStrSummary=null, blobStrVideo=null;
		...
		clobStrNarrative = getValueFromXml(userTutorial, "/Tutorial/TheFullContent");

		sb.delete(0, sb.length());
		clobStrSummary = getValueFromXml(userTutorial, "/Tutorial/summary");

		sb.delete(0, sb.length());
		blobStrVideo = getValueFromXml(userTutorial, "/Tutorial/video");


		Assert.assertNull("Failure: UserTutorial.narrative should not be marshalled, but it is.", clobStrNarrative);
		Assert.assertNull("Failure: UserTutorial.summary should not be marshalled, but it is.", clobStrSummary);
		Assert.assertNull("Failure: UserTutorial.video should not be marshalled, but it is.", blobStrVideo);
	}



	@Test
	public void testNonLobsPropertyIsMarshalled() throws XPathExpressionException, JAXBException
	{
		...
		Assert.assertEquals("Failure: Non-LOB property UserTutorial.getVideoType() is wrong.", "flac", videoType);
	}
