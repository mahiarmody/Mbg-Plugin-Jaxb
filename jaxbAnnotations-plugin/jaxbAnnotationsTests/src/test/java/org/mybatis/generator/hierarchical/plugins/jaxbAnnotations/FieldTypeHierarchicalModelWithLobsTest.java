...
the Hierarchical Model Type for the presence of Jaxb annotations when the Xml
access type specified is FIELD and LOBs fields are marshaled.

@author Mahiar Mody
*/
@RunWith(JUnit4.class)
public class FieldTypeHierarchicalModelWithLobsTest
{
	@Test
	...
	@Test
	public void testPrimaryKeyXmlAttributeIsInKeyClass()
	{
		Field fldUserTutorialId = null;

		Class<UserTutorialKey> cls = UserTutorialKey.class;
		...
		Assert.assertNotNull("@XmlAttribute annotation missing from UserTutorialKey.userTutorialId java class field", annt);

		String name = ((XmlAttribute) annt).name();
		boolean required = ((XmlAttribute) annt).required();

		Assert.assertEquals("name attribute of the @XmlAttribute annotation is wrong or missing in UserTutorialKey.userTutorialId java class field",
			"tutorialId", name);

		Assert.assertTrue("required attribute of @XmlAttribute annotation is wrong or missing in UserTutorialKey.userTutorialId java class field",
			required);
	}
	...
	public void testNarrativeFieldIsAnnotatedInBlobsClass()
	{
		...
		Assert.assertNotNull("UserTutorialWithBLOBs.narrative java field should be annotated, but it isn't.", annt);

		String name = ((XmlElement) annt).name();
		boolean required = ((XmlElement) annt).required();

		Assert.assertEquals("The name attribute of @XmlElement of UserTutorialWithBLOBs.narrative java field is wrong.",
			"TheFullContent", name);

		Assert.assertFalse("The required attribute of @XmlElement of UserTutorialWithBLOBs.narrative java field should be false but it isn't.",
			required);
	}
