...
Hierarchical Model Type, for the presence of Jaxb annotations when the Xml
access type specified is PROPERTY and LOBs fields are marshaled.

@author Mahiar Mody
*/
@RunWith(JUnit4.class)
public class PropertyTypeHierarchicalModelWithLobsTest
{
	@Test
	...
	@Test
	public void testPrimaryKeyMethodXmlAttributeIsInKeyClass()
	{
		...
		Assert.assertNotNull("@XmlAttribute annotation missing from UserTutorialKey.getUserTutorialId() method", annt);

		String name = ((XmlAttribute) annt).name();
		boolean required = ((XmlAttribute) annt).required();

		Assert.assertEquals("name attribute of the @XmlAttribute annotation is wrong or missing in UserTutorialKey.getUserTutorialId() method",
			"tutorialId", name);

		Assert.assertTrue("required attribute of @XmlAttribute annotation is wrong or missing in UserTutorialKey.getUserTutorialId() method",
			required);
	}
