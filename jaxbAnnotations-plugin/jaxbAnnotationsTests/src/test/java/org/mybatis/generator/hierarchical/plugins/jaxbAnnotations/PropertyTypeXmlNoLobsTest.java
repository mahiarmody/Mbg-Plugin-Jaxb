...
import hierarchical.propertyType.noLobs.modelDto.*; //these are our mbg generated compiled model classes that we want to test.
import hierarchical.propertyType.noLobs.clientDao.*;
...
public class PropertyTypeXmlNoLobsTest
{
	...
	public void testBlobsIgnored() throws MarshalException, XPathExpressionException, JAXBException
	{
		String blobStr = null;
		...
		try
		{
			...
			UserPhotos userPhotosWithBLOBs = lstUserPhotosWithBLOBs.get(0);

			blobStr = getValueFromXml(userPhotosWithBLOBs,
		...
		Assert.fail("Failure: No MarshalException thrown while marshaling BLOB class: UserPhotosWithBLOBs.");
	}



	@Test(expected = MarshalException.class)
	public void testClobsIgnored() throws MarshalException, XPathExpressionException, JAXBException
	{
		String clobStr = null;
		...
		try
		{
			...
			clobStr = getValueFromXml(userBlogWithBLOBs, "/userBlog/blogText");
		}
		finally
		{
			sqlSession.close();
		...
		Assert.fail("No MarshalException thrown while marshaling CLOB class: UserBlogWithBLOBs");
	}
	...
	public void testRecordWithBlobsNotAnnotated() throws MarshalException, XPathExpressionException, JAXBException
	{
		...
		try
		{
			...
			clobStr = getValueFromXml(userTutorialWithBLOBs, "/Tutorial/narrative");
		}
		finally
		{
			sqlSession.close();
