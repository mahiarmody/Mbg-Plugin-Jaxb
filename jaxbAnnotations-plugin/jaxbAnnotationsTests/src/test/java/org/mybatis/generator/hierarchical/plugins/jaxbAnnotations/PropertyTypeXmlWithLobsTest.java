...
import hierarchical.propertyType.withLobs.modelDto.*; //these are our mbg generated compiled model classes that we want to test.
import hierarchical.propertyType.withLobs.clientDao.*;
...
generated model classes when the Xml access type specified is PROPERTY and LOBs
fields are marshaled.

@author Mahiar Mody
*/
@RunWith(JUnit4.class)
public class PropertyTypeXmlWithLobsTest
{
	...
	public static void setUpBeforeClass() throws JAXBException, IOException
	{
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(Users.class, UserSkills.class ,UsersToSkillsKey.class,
			UserPhotos.class, UserPhotosWithBLOBs.class, UserTutorial.class, UserTutorialWithBLOBs.class,
			UserBlog.class, UserBlogWithBLOBs.class);
	...
	{
		UserPhotosWithBLOBs userPhotosWithBLOBs = null;
		...
		try
		{
			...
			List<UserPhotosWithBLOBs> lstUserPhotosWithBLOBs = mapper.selectByExampleWithBLOBs(ex);
			userPhotosWithBLOBs = lstUserPhotosWithBLOBs.get(0);
		}
		finally
		{
			sqlSession.close();
		...
		String blobStr = getValueFromXml(userPhotosWithBLOBs,
	...
	public void testClobsGenerated() throws XPathExpressionException, JAXBException
	{
		...
		try
		{
			...
			List<UserBlogWithBLOBs> lstUserBlogWithBLOBs = mapper.selectByExampleWithBLOBs(ex);
			UserBlog userBlogWithBLOBs = lstUserBlogWithBLOBs.get(0);

			clobStr = getValueFromXml(userBlogWithBLOBs, "/userBlogWithBLOBs/blogText");
		}
		finally
		{
			sqlSession.close();
		...
		Assert.assertEquals("Failure - CLOB datatype should be: 'bla bla bla', but it isn't.", "bla bla bla", clobStr);
	}



	@Test
	public void testRecordWithBlobsIsAnnotated() throws XPathExpressionException, JAXBException
	{
		UserTutorialWithBLOBs userTutorialWithBLOBs = null;
		...
		try
		{
			...
			userTutorialWithBLOBs = lstUserTutorialWithBLOBs.get(0);
		}
		finally
		{
			sqlSession.close();
		...
		String clobNarrative = getValueFromXml(userTutorialWithBLOBs, "/Tutorial/TheFullContent");
		sb.delete(0, sb.length());

		String clobSummary = getValueFromXml(userTutorialWithBLOBs, "/Tutorial/summary");
		sb.delete(0, sb.length());

		String base64Blob = getValueFromXml(userTutorialWithBLOBs, "/Tutorial/video");
	...
	public void testFieldToAttribute() throws XPathExpressionException, JAXBException
	{
		...
		usersKey.setUserId(Integer.valueOf(1));
		...
		try
		{
			...
			users = mapper.selectByPrimaryKey(usersKey);
		}
		finally
		{
			sqlSession.close();
	...
	{
		UserSkillsKey userSkillsKey = new UserSkillsKey();
		userSkillsKey.setSkillId(Short.valueOf((short)1));
		...
		try
		{
			...
			userSkills = mapper.selectByPrimaryKey(userSkillsKey);
		}
		finally
		{
			sqlSession.close();
	...
	@Test
	public void testInheritedJaxbComponents() throws XPathExpressionException, JAXBException
	{
		UserTutorialKey userTutorialKey = new UserTutorialKey();
		userTutorialKey.setUserTutorialId(Integer.valueOf(1));


		UserTutorialWithBLOBs userTutorialBlobs = null;

		SqlSession sqlSession = sqlSessionFactory.openSession();
		try
		{
			UserTutorialMapper mapper = sqlSession.getMapper(UserTutorialMapper.class);
			userTutorialBlobs = mapper.selectByPrimaryKey(userTutorialKey);
		}
		finally
		{
			sqlSession.close();
		...
		String tutorialId = getValueFromXml(userTutorialBlobs, "/Tutorial/@tutorialId");

		sb.delete(0, sb.length());
		String userId = getValueFromXml(userTutorialBlobs, "/Tutorial/userId");

		sb.delete(0, sb.length());
		String clobNarrative = getValueFromXml(userTutorialBlobs, "/Tutorial/TheFullContent");

		sb.delete(0, sb.length());
		String base64Blob = getValueFromXml(userTutorialBlobs, "/Tutorial/video");
		byte[] photoData = DatatypeConverter.parseBase64Binary(base64Blob);

		Assert.assertEquals("Failure - UserTutorialsWithBLOB does not inherit tutorialId attribute of UserTutorial.",
			tutorialId, "1");

		Assert.assertEquals("Failure - UserTutorialsWithBLOB does not inherit userId element of UserTutorial.",
			userId, "1");

		Assert.assertEquals("Failure marshalling CLOB column narrative:", clobNarrative, "The narrative goes here");
		Assert.assertNotNull("Failure marshalling BLOB column narrative:", photoData);
	}
}
